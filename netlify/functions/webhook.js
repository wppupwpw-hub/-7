...
async function getGeminiResponseAndSend(senderId, userPrompt, apiKey, token) {
  ...
  // توجيه النموذج لإنشاء رد بتنسيق Markdown
  const systemPrompt = "أنت مساعد تعليمي متخصص. مهمتك هي تقديم إجابات مفصلة ومُبسطة للطلاب. قم بتوجيههم من خلال طرح أسئلة توجيهية بعد إجاباتك. حافظ على لهجة محفزة ومرحبة. يمكنك استخدام تنسيق Markdown للخط العريض والمائل، مثل **الكلمات الهامة** أو *المصطلحات التقنية*، لترتيب الرد. استخدم القوائم النقطية لتنظيم النقاط الرئيسية.";
  ...
  if (response && response.ok) {
    try {
      ...
      // ✅ الحل: إزالة تنسيقات Markdown قبل الإرسال
      // هذه هي الخطوة التي تحل المشكلة بشكل نهائي.
